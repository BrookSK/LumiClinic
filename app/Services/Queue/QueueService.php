    ...
    private function sendPostTreatmentCare(int $clinicId, int $appointmentId, int $patientId, int $serviceId): void
    {
        ...
        try {
            ...
            try {
                ...
                $logToDb('Log insert failed: ' . $logErr->getMessage());
            }
        } catch (\Throwable $e) {
            $logToDb('WA Exception: ' . $e->getMessage());
        } finally {
            // Clean up session
