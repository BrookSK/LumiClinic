...
final class ProfessionalRepository
{
    ...
    {
        $sql = "
            SELECT id, clinic_id, name, specialty, allow_online_booking, status
            FROM professionals
    ...
    public function create(int $clinicId, string $name, ?string $specialty, bool $allowOnlineBooking): int
    {
        $sql = "
            INSERT INTO professionals (clinic_id, name, specialty, allow_online_booking, status, created_at)
            VALUES (:clinic_id, :name, :specialty, :allow_online_booking, 'active', NOW())
        ";
