...
<div class="lc-auth lc-auth--compact">
    <div class="lc-auth__panel lc-auth__panel--compact">
        <div class="lc-auth__brand">
            ...
            <div>
                <div class="lc-auth__title">Primeiro acesso</div>
                <div class="lc-auth__subtitle">Portal do Paciente</div>
            </div>
        </div>

        <?php if ($patientName): ?>
            <div class="lc-muted" style="margin-bottom:10px;">Olá, <?= htmlspecialchars((string)$patientName, ENT_QUOTES, 'UTF-8') ?>.</div>
        <?php endif; ?>

        <?php if ($email !== ''): ?>
            <div class="lc-muted" style="margin-bottom:10px;">Seu e-mail: <strong><?= htmlspecialchars((string)$email, ENT_QUOTES, 'UTF-8') ?></strong></div>
        <?php endif; ?>
        ...
        <form method="post" class="lc-form" action="/portal/register" autocomplete="off">
            <input type="hidden" name="_csrf" value="<?= htmlspecialchars($csrf, ENT_QUOTES, 'UTF-8') ?>" />
            <input type="hidden" name="token" value="<?= htmlspecialchars((string)$token, ENT_QUOTES, 'UTF-8') ?>" />

            <label class="lc-label">Defina uma senha</label>
            <input class="lc-input" type="password" name="password" required minlength="8" />

            <button class="lc-btn lc-btn--primary" type="submit">Salvar e entrar</button>
        </form>
