                    ...
                    <div class="lc-navgroup__children">
                        <div class="lc-nav__sub">
                            <?php if ($can('settings.read')): ?>
                                <?php
                                    $generalActive = ($isActive('/settings') && !$isActive('/settings/legal-documents'))
                                        || $isActive('/settings/terminology')
                                        || $isActive('/settings/operational')
                                        || $isActive('/settings/ai')
                                        || $isActive('/settings/whatsapp')
                    ...
                    <details class="lc-navgroup" <?= $patientsActive ? 'open' : '' ?>>
                        ...
                        <div class="lc-navgroup__children">
                            <div class="lc-nav__sub">
                                ...
                                <?php endif; ?>
                                <?= $navItem('/patients/birthdays', 'Aniversariantes / Follow-up', $ico['patients'], $isActive('/patients/birthdays') || $isActive('/patients/follow-up')) ?>
                            </div>
                        </div>
                    </details>
                <?php endif; ?>
                    ...
                    <details class="lc-navgroup" <?= $reportsActive ? 'open' : '' ?>>
                        ...
                        <div class="lc-navgroup__children">
                            <div class="lc-nav__sub">
                                ...
                                <?php endif; ?>
                                <?php if ($isSuperAdmin && $can('bi.read')): ?>
                                    <?= $navItem('/bi', 'BI', $ico['dashboard'], $isActiveSegment('/bi')) ?>
                                <?php endif; ?>
                                <?php if ($isSuperAdmin && $can('compliance.lgpd.read')): ?>
                                ...
                                <?php endif; ?>
                                <?php if ($isSuperAdmin && $can('compliance.incidents.read')): ?>
...
<?php if ($requiredLegalDocs !== [] && !str_starts_with($path, '/legal/sign') && !$isSuperAdmin): ?>
    <style>
...
?>
<a href="<?= htmlspecialchars($_waUrl, ENT_QUOTES, 'UTF-8') ?>" target="_blank" rel="noopener"
   style="position:fixed;bottom:20px;right:20px;z-index:9999;display:flex;align-items:center;gap:8px;padding:10px 18px;border-radius:50px;background:#25d366;color:#fff;font-size:13px;font-weight:700;text-decoration:none;box-shadow:0 4px 16px rgba(0,0,0,.2);transition:transform .15s,box-shadow .15s;"
   onmouseover="this.style.transform='scale(1.05)';this.style.boxShadow='0 6px 24px rgba(0,0,0,.25)'"
   onmouseout="this.style.transform='scale(1)';this.style.boxShadow='0 4px 16px rgba(0,0,0,.2)'"
