-- Migration: 0124_assign_marketing_calendar_permissions
...
-- Owner/Admin: allow read + manage
INSERT INTO role_permissions (clinic_id, role_id, permission_id, effect, created_at)
SELECT r.clinic_id, r.id, p.id, 'allow', NOW()
FROM roles r
INNER JOIN permissions p ON p.code IN ('marketing.calendar.read', 'marketing.calendar.manage')
...
-- Reception: allow somente read
INSERT INTO role_permissions (clinic_id, role_id, permission_id, effect, created_at)
SELECT r.clinic_id, r.id, p.id, 'allow', NOW()
FROM roles r
INNER JOIN permissions p ON p.code IN ('marketing.calendar.read')
WHERE r.deleted_at IS NULL
  AND p.deleted_at IS NULL
